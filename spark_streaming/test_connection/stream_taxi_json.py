...
spark = SparkSession.builder.appName("Stream Taxi Data").getOrCreate()
...
taxi_rides = taxi_rides.select(
    from_json(col("value"), taxi_schema).alias("data")
).select("data.*")

taxi_rides.writeStream.format("console").outputMode("append").start().awaitTermination()
