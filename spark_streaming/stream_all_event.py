...
import os
from streaming_function import *
...
AWS_S3_BUCKET = os.getenv("AWS_S3_BUCKET", 'streamify')
...
listen_events_writer = create_file_write_stream(
    listen_events,
    f"{S3_STORAGE_PATH}/{LISTEN_EVENTS_TOPIC}",
    f"{S3_STORAGE_PATH}/checkpoint/{LISTEN_EVENTS_TOPIC}",
)

page_view_events_writer = create_file_write_stream(
    page_view_events,
    f"{S3_STORAGE_PATH}/{PAGE_VIEW_EVENTS_TOPIC}",
    f"{S3_STORAGE_PATH}/checkpoint/{PAGE_VIEW_EVENTS_TOPIC}",
)

auth_events_writer = create_file_write_stream(
    auth_events,
    f"{S3_STORAGE_PATH}/{AUTH_EVENTS_TOPIC}",
    f"{S3_STORAGE_PATH}/checkpoint/{AUTH_EVENTS_TOPIC}",
)
