...
def create_kafka_read_stream(
    spark, kafka_address, kafka_port, topic, starting_offset="earliest"
):
    """
...
def process_stream(stream, stream_schema, topic):
    ...
    if topic in ["listen_events", "page_view_events"]:
        stream = stream.withColumn("song", string_decode("song")).withColumn(
            "artist", string_decode("artist")
        )
...
def create_file_write_stream(
    ...
    write_stream = (
        stream.writeStream.format(file_format)
